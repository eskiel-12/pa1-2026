...
<div class="container-fluid">
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.transportasi.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('nama') is-invalid @enderror" 
                           id="nama" name="nama" value="{{ old('nama', $item->nama) }}" placeholder="Nama transportasi" required>
                    @error('nama')
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('deskripsi') is-invalid @enderror" 
                              id="deskripsi" name="deskripsi" rows="5" placeholder="Deskripsi transportasi">{{ old('deskripsi', $item->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('destinasi_id') is-invalid @enderror" 
                        ...
                        @foreach($destinasis as $dest)
                            <option value="{{ $dest->id }}" {{ old('destinasi_id', $item->destinasi_id) == $dest->id ? 'selected' : '' }}>
                                {{ $dest->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control @error('lokasi') is-invalid @enderror" 
                               id="lokasi" name="lokasi" value="{{ old('lokasi', $item->lokasi) }}" placeholder="Lokasi transportasi">
                        @error('lokasi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
